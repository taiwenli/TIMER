## License GPL2.0
## Developed by Bo Li, bli@jimmy.harvard.edu
## Optimized by Taiwen Li, litaiwen@scu.edu.cn


TIMER <- function(e, cancer) {
  load("exdata/timer_pureImmune.Rdata")
  load("exdata/timer_geneMarker.Rdata")
  ...
  as.data.frame(apply(Y_cancer, 2, function(y) {GetFractions.Abbas(X_immune, y)})) %>%  
    ...
    mutate(cell_type=revalue(cell_type, transName)) %>%
    as.data.frame()
}
